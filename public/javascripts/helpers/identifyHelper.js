        ...
        */
        function deferredFactory(count) {
            var i = 0,
                deferreds = [];
            for (; i < count; i++) {
                deferreds[deferreds.length] = new Deferred();
            }
            return deferreds;
        }
        ...
        function cleanResponse(res) {
            var i = 0,
                ...
                results = [];
            for (; i < len; i++) {
                if (res[i][0]) results = results.concat(res[i][1]);
            }
            return results;
        }
        ...
        */
        IdentifyHelper.prototype.identifyHandler = function (map, layers) {
            ...
            */
            ...
            function updateIdParams(mapPoint) {
            ...
            function onMapClick(e) {
                ...
                idParams = updateIdParams(e.mapPoint);
                // use the identify layers described in config
                for (; i < len; i++) {
                    try {
                        idParams.layerIds = tasks[i].identifyLayers;
                        tasks[i].execute(idParams,
                                         deferredTasks[i].callback,
                                         deferredTasks[i].errback);
                    } catch(err) {
                        console.log(err);
